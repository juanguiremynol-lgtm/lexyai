...
export const ESTADOS_SIGNAL_EXPLANATION: Record<EstadosSignalClass, string> = {
  ...
  PROCESO_PRIVADO:
    "El proveedor de actuaciones AFIRMA que este proceso es privado y no expone su detalle («--- [ PROCESO PRIVADO ] ---»). Es una afirmación del proveedor, no un hecho comprobado: la registramos y no concluimos que el juzgado haya reservado el expediente — el abogado ya verificó un caso en el que la marca era incorrecta. La marca es por proceso, puede cambiar de un día para otro y habla sólo del canal de actuaciones: los estados deben publicarse siempre y se siguen leyendo y calificando aparte.",
  INDETERMINADO:
