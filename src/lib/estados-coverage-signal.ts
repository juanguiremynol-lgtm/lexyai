...
export const ESTADOS_SIGNAL_EXPLANATION: Record<EstadosSignalClass, string> = {
  ...
  PROCESO_PRIVADO:
    "El proveedor de actuaciones AFIRMA que este proceso es privado: la búsqueda lo devuelve con la leyenda «--- [ PROCESO PRIVADO ] ---» y el detalle responde «No se puede ver el detalle de un proceso privado». Es una afirmación del proveedor, no un hecho comprobado: la registramos y no concluimos que el juzgado haya reservado el expediente — el abogado ya verificó un caso en el que la marca era incorrecta. La causa de la marca no está declarada por el proveedor y nosotros no la interpretamos. Es una marca por proceso, puede cambiar de un día para otro y habla sólo del canal de actuaciones: los estados deben publicarse siempre y se siguen leyendo y calificando aparte. Aplica a cualquier área, no sólo a lo penal.",
  INDETERMINADO:
