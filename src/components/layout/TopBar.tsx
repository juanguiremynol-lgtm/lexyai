...
export function TopBar({ onNewFiling }: TopBarProps) {
  ...
  const handleSearch = (e: React.FormEvent) => {
    e.preventDefault();
    if (searchQuery.trim()) {
      navigate(`/filings?search=${encodeURIComponent(searchQuery.trim())}`);
    }
  };
    ...
    <header className="flex h-16 items-center justify-between border-b border-border bg-card px-4 lg:px-6">
      ...
      <div className="flex items-center gap-2">
        ...
        >
          ...
          <span className="hidden sm:inline">Nueva Radicación</span>
        </Button>
