 ...
 * including built-in defaults and configured external routes.
 */

import { Card, CardContent, CardDescription, CardHeader, CardTitle } from "@/components/ui/card";
import { Badge } from "@/components/ui/badge";
import { GitBranch, ArrowRight, CheckCircle2, AlertTriangle } from "lucide-react";
...
function resolveChain(workflow: string, scope: "ACTS" | "PUBS", routes: RouteRow[]): string[] {
  ...
  // External PRIMARY routes first (priority asc)
  ...
  // External FALLBACK routes last (priority asc)
  ...
  return (
    <Card className="border-slate-700 bg-slate-900/50">
      <CardHeader>
        ...
        <CardDescription>
          Orden de resolución de proveedores. ⚡ = externo PRIMARY, 🔄 = externo FALLBACK
        </CardDescription>
      </CardHeader>
      <CardContent>
                ...
                <div className="flex items-center gap-1 text-xs mb-1 flex-wrap">
                  ...
                  {actsChain.map((step, i) => (
                    <span key={i} className="flex items-center gap-1">
                      ...
                      <span className={step.startsWith("⚡") || step.startsWith("🔄")
