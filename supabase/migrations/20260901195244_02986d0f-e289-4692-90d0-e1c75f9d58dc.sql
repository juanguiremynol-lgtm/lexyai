...
BEGIN
  ...
  END LOOP;
  IF v_estados_provider IS NULL OR v_declared THEN v_class:='SIN_COBERTURA_DECLARADA'; v_basis:='COBERTURA_DECLARADA';
  ...
  -- JD1: a reserva is an ACTUACIONES fact. It may never override an estados
  -- verdict; it only explains an estados channel with no evidence at all.
  ELSIF v_reserva THEN v_class:='PROCESO_PRIVADO'; v_basis:='RESERVA_DECLARADA_POR_PROVEEDOR';
