CREATE OR REPLACE VIEW public.work_item_timeline_v AS
 SELECT a.work_item_id,
    ...
    a.id AS ref_id,
    jsonb_build_object('act_type', a.act_type, 'despacho', a.despacho, 'source', a.source, 'source_url', a.source_url, 'instancia_grado', COALESCE(a.instancia_grado, 'PRIMERA'::text), 'recurso_consecutivo', COALESCE(a.recurso_consecutivo, '00'::text), 'source_radicado', a.source_radicado) AS meta
...
UNION ALL
 SELECT p.work_item_id,
    ...
    jsonb_build_object('tipo', max(p.tipo_publicacion), 'despacho', max(p.despacho), 'pdf_url', max(p.pdf_url), 'fecha_desfijacion', max(p.fecha_desfijacion), 'source', max(p.source), 'attachment_count', count(*), 'instancia_grado', max(COALESCE(p.instancia_grado, 'PRIMERA'::text)), 'recurso_consecutivo', max(COALESCE(p.recurso_consecutivo, '00'::text)), 'source_radicado', max(p.source_radicado)) AS meta
   FROM work_item_publicaciones p
  WHERE COALESCE(p.is_archived, false) = false
  GROUP BY p.work_item_id, (COALESCE((p.fecha_fijacion AT TIME ZONE 'America/Bogota'::text)::date, (p.published_at AT TIME ZONE 'America/Bogota'::text)::date, p.created_at::date)), (lower(btrim(COALESCE(p.title, p.annotation, 'estado'::text)))), (COALESCE(p.recurso_consecutivo, '00'::text))
UNION ALL
