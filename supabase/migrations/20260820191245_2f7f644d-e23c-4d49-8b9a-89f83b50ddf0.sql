ALTER TABLE public.work_item_tombstones DROP COLUMN IF EXISTS archived_record;
ALTER TABLE public.work_item_tombstones ADD COLUMN IF NOT EXISTS partes text;
ALTER TABLE public.work_item_tombstones ADD COLUMN IF NOT EXISTS created_at timestamptz;

DROP FUNCTION IF EXISTS public.purge_soft_deleted_work_items(integer, boolean);
DROP FUNCTION IF EXISTS public.purge_soft_deleted_work_items(integer, boolean, boolean);
...
BEGIN
  FOR r IN
    SELECT w.id, w.radicado, w.title, w.workflow_type, w.authority_name,
           w.demandantes, w.demandados, w.created_at, w.deleted_at, w.delete_reason
      FROM public.work_items w
     WHERE w.deleted_at IS NOT NULL
       AND w.deleted_at < v_cutoff
     ORDER BY w.deleted_at
  LOOP
    v_partes := NULLIF(trim(both ' vs. ' FROM
      COALESCE(left(r.demandantes, 200), '') || ' vs. ' || COALESCE(left(r.demandados, 200), '')), '');
    ...
    IF p_dry_run THEN
      v_items := v_items || jsonb_build_object(
        ...
        'partes', v_partes, 'created_at', r.created_at,
        'deleted_at', r.deleted_at, 'delete_reason', r.delete_reason,
        'would_delete', v_counts, 'result', 'DRY_RUN'
      );
      CONTINUE;
    END IF;

    BEGIN
      -- 1. Tombstone FIRST. audit_logs is never touched, never cascaded into.
      INSERT INTO public.work_item_tombstones
        (id, radicado, titulo, workflow_type, despacho, partes, created_at,
         deleted_at, delete_reason, purged_at, counts)
      VALUES (r.id, r.radicado, r.title, r.workflow_type, r.authority_name, v_partes,
              r.created_at, r.deleted_at, r.delete_reason, now(), v_counts)
      ON CONFLICT (id) DO UPDATE SET purged_at = now(), counts = EXCLUDED.counts;

      -- 2. Verify before any deletion.
      SELECT true INTO v_ok FROM public.work_item_tombstones t WHERE t.id = r.id;
      IF p_simulate_verify_failure THEN v_ok := false; END IF;
      IF NOT COALESCE(v_ok, false) THEN
        RAISE EXCEPTION 'TOMBSTONE_VERIFY_FAILED for %', r.id;
      END IF;

      -- 3. Delete children, then the matter.
      v_deleted := '{}'::jsonb;
      ...
      v_items := v_items || jsonb_build_object(
        'id', r.id, 'radicado', r.radicado, 'titulo', r.title,
        'workflow_type', r.workflow_type, 'partes', v_partes,
    ...
    EXCEPTION WHEN OTHERS THEN
      ...
      v_items := v_items || jsonb_build_object(
        'id', r.id, 'radicado', r.radicado, 'titulo', r.title,
        'result', 'ROLLED_BACK', 'error', SQLERRM
      );
    END;
  END LOOP;
