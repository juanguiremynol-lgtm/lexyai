-- ===== Fase 2, Migración 3: relojes de fondo (caducidad y recurso de 1 año) =====
...
BEGIN
  ...
  LOOP
    ...
    -- Satisfacción: notificación del acto sancionatorio dentro del término.
    ...
    ELSIF v_today > v_deadline THEN
      -- Vencido sin notificación: se marca para atención. NO se declara la
      -- caducidad ni se mueve la etapa: eso lo decide el abogado.
      UPDATE public.work_item_deadlines
  ...
  ---------------------------------------------------------------------------
  -- B. Un año para decidir cada recurso (CPACA art. 52 inc. 2)
  ---------------------------------------------------------------------------
  FOR r IN
...
REVOKE ALL ON FUNCTION public.evaluate_gov_procedure_background_timers() FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.evaluate_gov_procedure_background_timers() TO service_role;

-- Diario a las 07:25 hora de Colombia (12:25 UTC), después de los eventos de PETICION.
SELECT cron.unschedule('gov-procedure-background-timers')
 WHERE EXISTS (SELECT 1 FROM cron.job WHERE jobname = 'gov-procedure-background-timers');

SELECT cron.schedule(
  'gov-procedure-background-timers',
  '25 12 * * *',
  $cron$SELECT public.evaluate_gov_procedure_background_timers();$cron$
);