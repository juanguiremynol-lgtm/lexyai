...
AS $function$
  SELECT CASE
    ...
    WHEN p_subtype = 'FALLO_SENTENCIA' THEN CASE p_workflow
      ...
      WHEN 'PENAL_906' THEN 'SENTENCIA_TRAMITE'
      ELSE 'ALEGATOS_SENTENCIA' END
    ELSE NULL
  END
$function$;

-- Los subtipos penales NO generan términos: el catálogo de plazos penales
-- (que corre desde fechas de audiencia, no desde fijación en estado) está
-- pendiente de especificación. email_subtype_deadline_type los deja en NULL
-- por su rama ELSE, de modo que ningún plazo civil se aplica a lo penal.