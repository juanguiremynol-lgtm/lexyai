...
BEGIN
  ...
  -- PENDING_UPSTREAM / PROVIDER_UNRESOLVED / CALLER_UNAUTHORIZED reads assert nothing.
  ELSIF jsonb_array_length(v_unmatched)>0 AND NOT v_read_complete THEN v_class:='LECTURA_NO_CONCLUYENTE';
  ...
  ELSIF v_acts>0 AND v_pubs=0 AND v_fij=0 AND NOT v_read_complete THEN v_class:='LECTURA_NO_CONCLUYENTE';
