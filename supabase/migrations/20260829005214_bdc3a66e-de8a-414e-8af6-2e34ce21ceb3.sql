...
BEGIN
  ...
  -- IR2: no automatic path may stop monitoring a matter. Absence of provider
  -- rows, silence and read failures describe the provider or us, never the
  -- matter. Only a person pauses. The refusal is recorded, not swallowed.
  IF p_new_state = 'PAUSED' AND NOT public.lifecycle_actor_is_human(p_actor) THEN
    INSERT INTO public.lifecycle_pause_refusals (work_item_id, attempted_state, actor, reason, metadata)
    VALUES (p_work_item_id, p_new_state::text, COALESCE(p_actor, 'SYSTEM'), p_reason,
            COALESCE(p_metadata, '{}'::jsonb) || jsonb_build_object('prev_state', v_prev));
    RETURN jsonb_build_object('ok', false, 'refused', true, 'prev_state', v_prev,
                              'attempted_state', p_new_state, 'actor', COALESCE(p_actor, 'SYSTEM'),
                              'error', 'AUTOMATIC_PAUSE_FORBIDDEN');
  END IF;
  ...
  UPDATE public.work_items SET
    ...
    monitoring_suspended_at = CASE WHEN p_new_state = 'ACTIVE' THEN NULL
                                   ELSE COALESCE(monitoring_suspended_at, v_now) END,
