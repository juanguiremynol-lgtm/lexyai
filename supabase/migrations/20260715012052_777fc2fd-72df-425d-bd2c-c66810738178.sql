  ...
  LOOP
    BEGIN
      ...
      ELSE
        ...
        ELSE
          -- Fill-only enrichment path. Existing keys always win.
          UPDATE work_item_acts SET
            last_seen_at = now(),
            scrape_date  = COALESCE((rec->>'scrape_date')::timestamptz, scrape_date),
            raw_data     = v_incoming_raw || COALESCE(work_item_acts.raw_data, '{}'::jsonb),
          ...
          -- Count as "enriched" if incoming brought at least one NEW top-level key,
          -- or filled a previously NULL dedicated column.
