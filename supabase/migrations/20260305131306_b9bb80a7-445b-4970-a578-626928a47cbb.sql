    ...
    BEGIN
      INSERT INTO alert_instances (
        ...
        jsonb_build_object(
          ...
          'annotation', COALESCE(LEFT(NEW.annotation, 200), LEFT(NEW.event_summary, 200), ''),
          'source', COALESCE(NEW.source, ''),
          'despacho', COALESCE(v_work_item.authority_name, '')
        )
      );
    EXCEPTION WHEN OTHERS THEN
      BEGIN
        INSERT INTO sync_traces (
          work_item_id, organization_id, step, provider,
          success, error_code, error_message, metadata, created_at
        ) VALUES (
          NEW.work_item_id,
          COALESCE(NEW.organization_id, v_work_item.organization_id),
          'ALERT_INSERT_FAILED', 'trigger',
          false, 'ALERT_TRIGGER_ERROR', SQLERRM,
          jsonb_build_object(
