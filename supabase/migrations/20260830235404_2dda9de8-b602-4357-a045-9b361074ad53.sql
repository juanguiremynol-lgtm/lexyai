...
WITH att AS (
  ...
  -- A routing skip is not a read: the source was never asked. It must not be
  -- graded at all (it used to land in the catch-all 'error' bucket).
  WHERE lower(coalesce(a.value->>'status','')) <> 'skipped'
