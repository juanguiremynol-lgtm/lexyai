...
AS $function$
  SELECT CASE
    ...
    -- Ejecutivo (CGP): evaluado antes de FALLO_SENTENCIA / TRASLADO porque
    -- comparte vocabulario ("sentencia que ordena seguir adelante", "traslado
    -- de excepciones").
    WHEN COALESCE(p_subject,'') ~* 'mandamiento +(ejecutivo +)?(de +)?pago|l[ií]bra?se? +mandamiento|libra +mandamiento' THEN 'MANDAMIENTO_PAGO'
    WHEN COALESCE(p_subject,'') ~* 'seguir +adelante' THEN 'SEGUIR_ADELANTE'
    WHEN COALESCE(p_subject,'') ~* 'remate|adjudicaci[oó]n +del? +bien' THEN 'REMATE'
    WHEN COALESCE(p_subject,'') ~* 'aval[uú]o|aval[uú]a' THEN 'AVALUO'
    WHEN COALESCE(p_subject,'') ~* 'liquidaci[oó]n +del? +cr[eé]dito|liquidaci[oó]n +de +costas' THEN 'LIQUIDACION_CREDITO'
    WHEN COALESCE(p_subject,'') ~* 'excepciones +de +m[eé]rito|traslado +de +(las +)?excepciones' THEN 'EXCEPCIONES_MERITO'
...
AS $function$
  SELECT CASE p_subtype
    ...
    WHEN 'MANDAMIENTO_PAGO' THEN 'Mandamiento de pago'
    WHEN 'SEGUIR_ADELANTE' THEN 'Ordena seguir adelante la ejecución'
    WHEN 'EXCEPCIONES_MERITO' THEN 'Excepciones de mérito'
    WHEN 'LIQUIDACION_CREDITO' THEN 'Liquidación del crédito'
    WHEN 'AVALUO' THEN 'Avalúo de bienes'
    WHEN 'REMATE' THEN 'Remate'
...
AS $function$
  SELECT CASE
    ...
    -- Ejecutivo (CGP): etapas propias del apremio.
    WHEN p_subtype = 'MANDAMIENTO_PAGO' THEN 'MANDAMIENTO_PAGO'
    WHEN p_subtype = 'SEGUIR_ADELANTE' THEN 'SEGUIR_ADELANTE'
    WHEN p_subtype = 'EXCEPCIONES_MERITO' THEN 'EXCEPCIONES_MERITO'
    WHEN p_subtype = 'LIQUIDACION_CREDITO' THEN 'LIQUIDACION_CREDITO'
    WHEN p_subtype = 'AVALUO' THEN 'AVALUO_BIENES'
    WHEN p_subtype = 'REMATE' THEN 'REMATE'
