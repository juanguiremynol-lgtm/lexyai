-- Update platform_verification_snapshot to include usage counts
...
BEGIN
  ...
  -- 5) Job runs for scheduled operations (schema-safe)
  IF v_job_runs_exists THEN
    -- Get last successful run - use dynamic SQL only if metadata exists
    IF v_job_runs_has_metadata THEN
      SELECT id, job_name, status, finished_at, duration_ms, processed_count, 
