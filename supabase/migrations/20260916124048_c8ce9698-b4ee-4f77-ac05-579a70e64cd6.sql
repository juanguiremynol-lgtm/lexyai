...
AS $function$
  ...
  -- MA1 — el despacho es los primeros 12 dígitos del radicado. Un despacho que
  -- entrega para un asunto y nunca para otro es un hecho sobre el radicado; uno
  -- que no entrega para ninguno es un hecho sobre el despacho. Solo se comparan
  -- asuntos de la misma cadena de fuente.
  peers AS (
...
-- MA2 — cadencia, no pausa. Un asunto que lleva _streak_threshold días
-- consecutivos de PENDING_UPSTREAM y jamás ha recibido una fila de esa fuente
-- se consulta una vez cada _probe_every_days. Sigue monitoreado; sólo se
-- pregunta menos. Umbral derivado de la historia: de 33 asuntos que sí
-- recibieron publicaciones, el retraso máximo entre la primera consulta y la
-- primera fila fue de 12 días; 14 queda por encima de todo lo observado.
CREATE OR REPLACE FUNCTION public.estados_probe_deferred_ids(
  _source text,
  _streak_threshold int DEFAULT 14,
  _probe_every_days int DEFAULT 7
)
RETURNS TABLE(work_item_id uuid)
LANGUAGE sql
STABLE
SET search_path TO 'public'
AS $function$
  SELECT c.work_item_id
  FROM public.source_coverage_persistence(_source, 60) c
  WHERE c.gap_class = 'NEVER_ANSWERED'
    AND coalesce(c.rows_ever, 0) = 0
    AND c.consecutive_days >= _streak_threshold
    AND c.last_day > ((now() AT TIME ZONE 'America/Bogota')::date - _probe_every_days)
$function$;

GRANT EXECUTE ON FUNCTION public.estados_probe_deferred_ids(text, int, int) TO authenticated, service_role;
