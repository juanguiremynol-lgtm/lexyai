...
var list_deadlines_default = defineTool9({
  ...
  handler: async ({ status, radicado, limit }, ctx) => {
    ...
    const { data: items } = ids.length ? await sb.from("work_items").select("id, radicado, title, workflow_type, authority_name, demandantes, demandados").in("id", ids).is("deleted_at", null) : { data: [] };
    const byId = new Map(
      (items ?? []).map(
        (i) => [i.id, i]
      )
