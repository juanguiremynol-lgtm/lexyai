  ...
  try {
    ...
    for (const { user_id, organization_id } of userOrgPairs) {
      try {
        ...
        // LB1 — the embedded predicate reproduces `v_monitored_work_items`:
        // deleted matters are out, everything alive is monitored. There is no
        // hidden or paused state to filter for.
        const { data: novedadActs } = await supabase
          .from("work_item_acts")
          .select("id, work_item_id, description, act_date, detected_at, discovery_type, retro_gap_days, work_items!inner(radicado, title, deleted_at, monitoring_enabled)")
          .eq("organization_id", organization_id)
        ...
        const { data: novedadPubs } = await supabase
          ...
          .select("id, work_item_id, tipo_publicacion, fecha_fijacion, fecha_desfijacion, detected_at, discovery_type, retro_gap_days, work_items!inner(radicado, title, deleted_at, monitoring_enabled)")
          .eq("organization_id", organization_id)
          .eq("is_notifiable", true)
          .is("work_items.deleted_at", null)
          .eq("work_items.monitoring_enabled", true)
