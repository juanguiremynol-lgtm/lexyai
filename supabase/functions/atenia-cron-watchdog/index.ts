...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // IR1(a): the ghost detector was DELETED in full — no detection, no
    // observation, no GHOST_SUSPECTED row. A matter that receives no updates
    // is not added to Andromeda in the first place, and every despacho is
    // obliged to publish estados. The system forms no opinion about whether a
    // work item deserves to be monitored.
