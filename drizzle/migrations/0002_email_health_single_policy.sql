-- 1. Separate "last renewal ATTEMPT" from "last renewal SUCCESS".
...
UPDATE public.user_email_connections
   SET last_refresh_success_at = last_refresh_at
 WHERE last_refresh_success_at IS NULL
   AND last_refresh_outcome = 'SUCCESS'
   AND last_refresh_at IS NOT NULL;

-- 2. Detector rewritten to the single shared policy:
--    * an externally revoked connection (failure_code present) IS evaluated;
--      only a voluntary disconnect (failure_code null) is skipped;
--    * "24 h sin renovar" uses last_refresh_success_at, never the attempt
--      stamp, and no longer shares its headline with the failure-count case.
...
BEGIN
  ...
  LOOP
    ...
    ELSIF r.last_refresh_outcome = 'FAILED' AND r.refresh_failure_count >= 3 THEN
      v_type := 'EMAIL_CONEXION_ERROR';
      v_sev := 'CRITICAL';
      v_title := 'La renovacion del buzon fallo de forma consecutiva';
      v_msg := 'La renovacion de ' || COALESCE(r.ms_account_email, '(sin direccion)') ||
               ' fallo ' || r.refresh_failure_count ||
               ' veces seguidas. La ultima renovacion exitosa fue el ' ||
               COALESCE(to_char(v_last_ok AT TIME ZONE 'America/Bogota', 'DD/MM/YYYY HH24:MI'), 'nunca') || '.';
